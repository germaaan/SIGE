...
total$Survived <- ifelse(total$Survived ==1,'yes','nope')
...
model2 = train(modelo, data=train, method="gbm", distribution="adaboost", verbose=F, tuneGrid=data.frame(.n.trees=5000, .shrinkage=0.01, .interaction.depth=1, .n.minobsinnode=1))
...
gbmFit2 <- train(modelo, data = train,
                 method = "gbm",
...
sensitivity(as.factor(prediccion), as.factor(test$Survived))

confusionMatrix(as.factor(prediccion), test$Survived)
