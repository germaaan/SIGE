...
# Clasificamos animales por tener o no tener nombre
...
# Procesamos edad para homogeneizarla a valor en días
...
for (iter in 1:20) {
  ...
  if (min_logloss < mejor_logloss) {
    mejor_logloss = min_logloss
    mejor_logloss_index = min_logloss_index
    mejores_parametros = parametros
  }
}

num_rondas = mejor_logloss_index

# Ajustar modelo en el conjunto de validacion
validacion <- xgboost(param=mejores_parametros, data=train.x[-in.train, ], label=train.y[-in.train], 
                      nrounds=num_rondas, nthread=6)

# Ajustar modelo en el conjunto de entrenamiento completo
ajuste <- xgboost(param=mejores_parametros, data=train.x, label=train.y, nrounds=num_rondas, nthread=6)
